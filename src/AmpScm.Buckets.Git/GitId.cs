...
#if !NETFRAMEWORK
            if (format!.StartsWith('x') && int.TryParse(format.AsSpan(1), NumberStyles.None, CultureInfo.InvariantCulture, out var xLen))
#else
            if (format!.StartsWith("x", StringComparison.Ordinal) && int.TryParse(format.Substring(1), NumberStyles.None, CultureInfo.InvariantCulture, out var xLen))
#endif
                return ToString().Substring(0, xLen);
#if !NETFRAMEWORK
            else if (format.StartsWith('X') && int.TryParse(format.AsSpan(1), NumberStyles.None, CultureInfo.InvariantCulture, out var xlen))
#else
            else if (format.StartsWith("X", StringComparison.Ordinal) && int.TryParse(format.Substring(1), NumberStyles.None, CultureInfo.InvariantCulture, out var xlen))
#endif
